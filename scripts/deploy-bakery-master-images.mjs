...
if (manifest.phase !== "9.13.12" || manifest.images.length !== 11 || manifest.images.some((e) => !manifest.categories.includes(e.category) || e.lifecycle !== "PENDING_REVIEW" || e.responsive_variants.length !== 6)) throw new Error("Unexpected Phase 9.13.12 manifest.");
...
for (const entry of manifest.images) {
  ...
  const paths = entry.responsive_variants.map((v) => v.storage_path);
  ...
  for (const existing of existingVersions) {
    const variant = entry.responsive_variants.find((v) => v.storage_path === existing.storage_path);
  ...
  for (const variant of entry.responsive_variants) {
    if (existingVersions.some((e) => e.storage_path === variant.storage_path)) continue;
    ...
    if (stored) {
      if (createHash("sha256").update(Buffer.from(await stored.arrayBuffer())).digest("hex") !== variant.checksum_sha256) throw new Error(`Existing Storage object differs for ${entry.dish_name}.`);
    } else {
  ...
  if (pendingError) throw pendingError; uploaded += 1; console.log(`${entry.dish_name}: 6 variants uploaded, PENDING_REVIEW`);
}
const paths = manifest.images.flatMap((e) => e.responsive_variants.map((v) => v.storage_path));
...
if (verified.length !== paths.length || verified.some((r) => r.status !== "PENDING_REVIEW" || r.smart_image.status !== "PENDING_REVIEW" || r.smart_image.current_version !== 1)) throw new Error("Remote Phase 9.13.12 certification failed.");
