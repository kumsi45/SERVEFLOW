...
if (manifest.phase !== "9.13.10" || manifest.images.length !== 14 || manifest.images.some((entry) => !manifest.categories.includes(entry.category) || entry.lifecycle !== "PENDING_REVIEW" || entry.responsive_variants.length !== 6)) throw new Error("Unexpected Phase 9.13.10 manifest.");
...
if (verified.length !== paths.length || verified.some((row) => row.status !== "PENDING_REVIEW" || row.smart_image.status !== "PENDING_REVIEW" || row.smart_image.current_version !== 1)) throw new Error("Remote Phase 9.13.10 certification failed.");
