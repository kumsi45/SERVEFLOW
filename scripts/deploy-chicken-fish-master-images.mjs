...
if (manifest.phase !== "9.13.5" || manifest.images.length !== 9) throw new Error("Unexpected Phase 9.13.5 manifest.");
if (manifest.images.some((entry) => !manifest.categories.includes(entry.category) || entry.lifecycle !== "PENDING_REVIEW" || entry.responsive_variants.length !== 6)) throw new Error("Manifest category, lifecycle, or responsive contract failed.");
...
const names = manifest.images.map((entry) => entry.dish_name);
const { data: items, error: itemError } = await supabase.from("serveflow_master_menu_items").select("id,name").in("name", names);
if (itemError) throw itemError;
const { data: mappings, error: mappingError } = await supabase.from("serveflow_smart_menu_library_items").select("item_id").eq("library_id", library.id).in("item_id", items.map((item) => item.id));
if (mappingError) throw mappingError;
const mappedIds = new Set(mappings.map((mapping) => mapping.item_id));
const scopedItems = items.filter((item) => mappedIds.has(item.id));
if (scopedItems.length !== 9) throw new Error(`Expected 9 canonical Restaurant items, found ${scopedItems.length}.`);
const itemByName = new Map(scopedItems.map((item) => [item.name, item.id]));
const { data: images, error: imageError } = await supabase.from("serveflow_smart_menu_images").select("id,item_id,base_storage_path,status,current_version").eq("library_id", library.id).in("item_id", scopedItems.map((item) => item.id));
if (imageError) throw imageError;
if (images.length !== 9) throw new Error(`Expected 9 Smart Image identities, found ${images.length}.`);
const imageByItem = new Map(images.map((image) => [image.item_id, image]));

let uploaded = 0;
let skipped = 0;
for (const entry of manifest.images) {
  const itemId = itemByName.get(entry.dish_name);
  const image = imageByItem.get(itemId);
  if (!image || image.base_storage_path !== entry.base_storage_path) throw new Error(`Unexpected Smart Image identity for ${entry.dish_name}.`);
  ...
  if (existingVersions.length === paths.length) {
    for (const variant of entry.responsive_variants) {
      const existing = existingVersions.find((row) => row.storage_path === variant.storage_path);
      if (!existing || existing.checksum_sha256 !== variant.checksum_sha256 || existing.status !== "PENDING_REVIEW") throw new Error(`Existing immutable variant differs for ${entry.dish_name}.`);
    }
    skipped += 1;
    continue;
  }
