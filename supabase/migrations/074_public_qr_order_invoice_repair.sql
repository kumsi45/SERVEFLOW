-- SERVEFLOW Phase 5.1 Public QR invoice repair.
-- Restores invoice-backed public QR order creation and repairs unverified
-- public QR orders that were created without cashier-visible invoices.
...
with target_orders as (
  select orders.*
  from public.orders orders
  where orders.order_source = 'public_qr'
    and orders.status::text in ('pending_payment', 'paid')
...
created_invoices as (
  insert into public.order_invoices (restaurant_id, order_id, invoice_number, status, total_price, payment_method, created_at, updated_at)
  select
    target_orders.restaurant_id,
    target_orders.id,
    1,
    'pending',
    target_orders.total_price,
    target_orders.payment_method,
    target_orders.created_at,
    now()
  from target_orders
  returning id, restaurant_id, order_id
),
linked_items as (
  update public.order_items items
  set
    invoice_id = created_invoices.id,
    kitchen_status = case when items.kitchen_status = 'paid' then 'held' else items.kitchen_status end
  from created_invoices
  where items.restaurant_id = created_invoices.restaurant_id
    and items.order_id = created_invoices.order_id
    and items.invoice_id is null
  returning items.order_id
)
...
where exists (
  select 1
  from created_invoices
  where created_invoices.restaurant_id = orders.restaurant_id
    and created_invoices.order_id = orders.id
);
