-- Phase A2: a waiter may see only active tables with an active assignment to
-- their authenticated waiter membership. There is deliberately no all-table
-- fallback when the waiter has zero assignments.
...
begin
  ...
  with visible_tables as (
    select
      ...
      current_waiter.display_name as assigned_staff_name
    from public.restaurant_tables tables
    join public.restaurant_table_waiter_assignments assignments
...
comment on function public.get_waiter_dashboard_tables(text) is
  'Returns only active tables actively assigned to the authenticated active waiter in the requested restaurant. Zero assignments returns zero rows.';
