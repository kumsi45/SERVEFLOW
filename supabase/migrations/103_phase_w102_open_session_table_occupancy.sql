-- Table occupancy follows dining_session_status, never a historical order status.
...
declare
  ...
  normalized_identifier text := lower(trim(target_restaurant_slug));
  slugified_identifier text := trim(both '-' from regexp_replace(lower(trim(target_restaurant_slug)), '[^a-z0-9]+', '-', 'g'));
  has_assignments boolean := false;
begin
  if auth.uid() is null then raise exception 'Authentication is required to view waiter tables.'; end if;

  select * into target_restaurant
  from public.restaurants restaurants
  where restaurants.active
    and (
      restaurants.slug = normalized_identifier
      or restaurants.id::text = normalized_identifier
      or lower(trim(restaurants.name)) = normalized_identifier
      or restaurants.slug = slugified_identifier
    )
  order by case
    when restaurants.slug = normalized_identifier then 0
    when restaurants.id::text = normalized_identifier then 1
    when lower(trim(restaurants.name)) = normalized_identifier then 2
    else 3 end
  limit 1;

  if target_restaurant.id is null then raise exception 'Restaurant not found.'; end if;

  select * into current_waiter
  ...
  where staff.restaurant_id = target_restaurant.id
    ...
    and staff.active
  limit 1;
  if current_waiter.id is null then raise exception 'Active waiter membership not found for this restaurant.'; end if;

  select exists (
    select 1 from public.restaurant_table_waiter_assignments assignments
  ...
  with visible_tables as (
    select tables.*, assignments.waiter_staff_id as assigned_staff_id, assigned_staff.display_name as assigned_staff_name
    ...
    left join public.restaurant_staff assigned_staff
     ...
     and assigned_staff.active
    where tables.restaurant_id = target_restaurant.id
      and tables.active
      and (not has_assignments or assignments.waiter_staff_id = current_waiter.id)
  ), active_orders as (
  ...
  select
    ...
    case when active_orders.id is null then 'available' when active_orders.order_source = 'public_qr' then 'qr_ordering' else 'occupied' end,
    active_orders.id,
  ...
  from visible_tables
  left join active_orders on active_orders.table_number = visible_tables.table_number::text
