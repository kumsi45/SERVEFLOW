...
export function CashierIcon({ name }: { name: CashierIconName }) {
  const paths: Record<CashierIconName, ReactNode> = {
    ...
    active: <><rect x="3" y="4" width="18" height="16" rx="3"/><path d="M7 8h10M7 12h6"/></>,
    collection: <><path d="M4 7h16v11H4zM8 7V5h8v2"/><path d="M9 13h6"/></>,
    ...
    <header className="cd-header">
      ...
      <label className="cd-header-search">
        ...
        <input
          ...
          placeholder="Search service location, customer, invoice, order or phone"
          aria-label="Search by service location, customer, invoice, order number, or phone number"
        />
        <kbd>Ctrl K</kbd>
      </label>
      <div className="cd-header-right">
        ...
        <button type="button" className="cd-signout-btn" onClick={onSignOut}>
          <CashierIcon name="user" /> <span>Sign Out</span>
        </button>
      </div>
    </header>
  );
