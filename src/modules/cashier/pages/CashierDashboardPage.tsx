...
function fmtInvoiceLabel(order: Pick<CashierOrder, "invoiceDisplayNumber" | "invoiceNumber">) {
  return order.invoiceDisplayNumber ?? `Inv ${order.invoiceNumber ?? 1}`;
}

function fmtSessionLabel(session: Pick<DiningSessionSummary, "diningSessionDisplayNumber" | "diningSessionId">) {
...
function creatorLabel(order: Pick<CashierOrder, "invoiceCreatorName" | "invoiceSource" | "waiterName">) {
  return order.invoiceCreatorName || order.waiterName || sourceLabel(order.invoiceSource);
}
...
type BillHistory = { dining_session_id: string; print_count: number; printed_at: string };

const PAYMENT_METHODS = ["Cash", "Telebirr", "CBE Birr", "Mobile Banking", "Card", "Chapa"];
...
function normalizeFinalBillPayload(value: unknown): FinalDiningBillModel {
  ...
  return {
    bill: {
      ...
      format: format === "58mm" || format === "a4" || format === "browser" ? format : "80mm",
    ...
    restaurant: {
      ...
      vatRegistrationNumber: restaurant.vat_registration_number ? String(restaurant.vat_registration_number) : null,
    ...
    items: Array.isArray(root.items) ? root.items.map((item) => {
    ...
    payments: Array.isArray(root.payments) ? root.payments.map((payment) => {
...
function customerTypeLabel(session: DiningSessionSummary) {
  const sources = new Set(session.batches.map((batch) => batch.invoiceSource ?? batch.orderSource));
  if (sources.has("public_qr") && sources.size > 1) return "QR + Staff Assisted";
...
function buildFinalBillReviewModel(session: DiningSessionSummary, restaurant: CashierRestaurant, cashierName: string, format: FinalBillFormat): FinalDiningBillModel {
  const grouped = new Map<string, FinalBillLineItem>();
  for (const batch of session.batches) for (const item of batch.items) {
    const key = `${item.name}:${item.price}`;
    const current = grouped.get(key) ?? { name: item.name, quantity: 0, unitPrice: item.price, total: 0 };
  ...
  for (const batch of session.batches.filter((batch) => batch.invoiceStatus === "verified")) methods.set(batch.paymentMethod || "Other", (methods.get(batch.paymentMethod || "Other") ?? 0) + batch.totalPrice);
  return {
    bill: { id: "", billNumber: "", receiptNumber: "", diningSessionId: session.diningSessionId, diningSessionNumber: "", tableNumber: session.tableNumber, customerName: session.customerName, waiterName: session.waiterName, cashierName, printedAt: new Date().toISOString(), printCount: 0, format, pdfPath: null, status: "preview" },
    restaurant: { name: restaurant.name, logoUrl: restaurant.logoUrl, tinNumber: null, vatRegistrationNumber: null, phone: null, address: null, website: null },
    items: [...grouped.values()], totals: { subtotal, vatRate: .15, vatAmount: total - subtotal, serviceChargeRate: 0, serviceChargeAmount: 0, discountAmount: 0, grandTotal: total },
    payments: [...methods].map(([method, amount]) => ({ method, amount })),
  };
}

function chunkBillItems(items: FinalBillLineItem[], format: FinalBillFormat) {
  const perPage = format === "browser" ? 24 : format === "a4" ? 28 : format === "58mm" ? 10 : 16;
...
function buildFinalBillPrintHtml(model: FinalDiningBillModel) {
  ...
  const width = model.bill.format === "58mm" ? "58mm" : model.bill.format === "80mm" ? "80mm" : isBrowser ? "100%" : "210mm";
  const pageSize = isA4 ? "A4" : model.bill.format === "58mm" ? "58mm auto" : model.bill.format === "80mm" ? "80mm auto" : "auto";
  const printedAt = new Date(model.bill.printedAt);
  const date = printedAt.toLocaleDateString("en-US", { year: "numeric", month: "short", day: "2-digit" });
  const time = printedAt.toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit" });
  const totalPaid = model.payments.reduce((sum, payment) => sum + payment.amount, 0);
  const paymentAmounts = new Map(model.payments.map((payment) => [payment.method.toLowerCase(), payment.amount]));
  const paymentBreakdown = ["Cash", "Telebirr", "CBE Birr", "Card", "Chapa", "Other"].map((method) => ({
    method,
...
function paymentScreenshotPath(restaurantId: string, invoiceId: string, file: File) {
...
function isDigitalPayment(order: CashierOrder) {
  return order.invoiceStatus === "verified" && order.paymentMethod !== "Cash";
}
...
function isAwaitingCollection(order: CashierOrder) {
  return order.invoiceStatus === "verified" && (order.status === "ready" || order.status === "paid");
}

function isCompletedOrder(order: CashierOrder) {
  return order.diningSessionStatus === "closed" || order.diningSessionStatus === "checked_out";
}

function isUnpaidPayment(order: CashierOrder) {
  return !isCompletedOrder(order) && order.invoiceStatus === "pending";
}

function isVerifiablePayment(order: CashierOrder) {
  return !isCompletedOrder(order) && (order.invoiceStatus === "pending" || order.invoiceStatus === "paid");
}
...
function buildDiningSessionSummaries(sessionOrders: CashierOrder[]) {
  ...
  for (const order of sessionOrders) {
    ...
    const current = sessions.get(diningSessionId) ?? {
      diningSessionId,
      diningSessionDisplayNumber: order.diningSessionDisplayNumber ?? order.displayNumber ?? null,
    ...
    current.verifiedTotal += order.invoiceStatus === "verified" ? order.totalPrice : 0;
    if (order.invoiceStatus !== "verified" && order.invoiceStatus !== "cancelled" && order.invoiceStatus !== "refunded") current.pendingCount += 1;
    current.itemCount += order.items.length;
    current.incompleteItemCount += order.items.filter((item) => item.kitchenStatus !== "completed").length;
    if (new Date(order.createdAt).getTime() > new Date(current.latestAt).getTime()) current.latestAt = order.createdAt;
    if (new Date(order.createdAt).getTime() < new Date(current.createdAt).getTime()) current.createdAt = order.createdAt;
    if (!current.waiterName && order.waiterName) current.waiterName = order.waiterName;
    if (!current.customerName && order.customerName) current.customerName = order.customerName;
    sessions.set(diningSessionId, current);
  }
  return [...sessions.values()].sort((left, right) => new Date(right.latestAt).getTime() - new Date(left.latestAt).getTime());
}
...
function OrderDrawer({
  ...
  const isVerified = order.invoiceStatus === "verified";
    ...
    <>
      ...
      <aside className="cd-drawer" role="dialog" aria-modal="true" aria-label="Order details">
        <div className="cd-drawer-header">
          <div>
            <div className="cd-drawer-title">{fmtOrderLabel(order)} · {fmtInvoiceLabel(order)}</div>
        ...
        <div className="cd-drawer-body">
          <div className="cd-drawer-detail-grid">
            <div className="cd-drawer-detail"><div className="cd-drawer-detail-label">Payment Status</div><div className="cd-drawer-detail-value">{statusLabel(order.invoiceStatus || "pending")}</div></div>
            <div className="cd-drawer-detail"><div className="cd-drawer-detail-label">Kitchen Status</div><div className="cd-drawer-detail-value">{statusLabel(order.invoiceKitchenStatus || "waiting_payment")}</div></div>
            <div className="cd-drawer-detail"><div className="cd-drawer-detail-label">Created By</div><div className="cd-drawer-detail-value">{creatorLabel(order)}</div></div>
            ...
            <div className="cd-drawer-detail"><div className="cd-drawer-detail-label">Verified By</div><div className="cd-drawer-detail-value">{order.invoiceVerifiedByName || "-"}</div></div>
          </div>
          {(isPending || order.invoiceStatus === "paid" || isRejected) ? (
            <div className="cd-payment-verification-box">
              <div className="cd-drawer-section-title">Payment Verification</div>
              <div className="cd-payment-fields">
                {isDigital ? (
                  <>
                    <label><span>Reference Number</span><input value={paymentReference} onChange={(event) => onPaymentReferenceChange(event.target.value)} maxLength={120} /></label>
                    <label><span>Transaction ID</span><input value={paymentTransactionId} onChange={(event) => onPaymentTransactionIdChange(event.target.value)} maxLength={120} /></label>
                    <label className="wide">
                      <span>Payment Screenshot</span>
                      <input type="file" accept="image/*" onChange={(event) => onPaymentScreenshotFileChange(event.target.files?.[0] ?? null)} />
                    </label>
                    {paymentScreenshotPreviewUrl ? (
                      <a className="cd-payment-preview" href={paymentScreenshotPreviewUrl} target="_blank" rel="noreferrer">
                        <img src={paymentScreenshotPreviewUrl} alt="Payment screenshot preview" />
                      </a>
                    ) : paymentScreenshotUrl ? (
                      <div className="cd-empty-sub wide">Stored screenshot path ready for verification.</div>
                    ) : null}
                    {duplicateReferenceNotice ? (
                      <div className="cd-payment-duplicate wide">
                        <span>{duplicateReferenceNotice}</span>
                        <label>
                          <input type="checkbox" checked={ownerDuplicateOverride} onChange={(event) => onOwnerDuplicateOverrideChange(event.target.checked)} />
                  ...
                  <div className="cd-cash-confirmation">Cash requires cashier confirmation before kitchen release.</div>
                )}
                <label className="wide"><span>Cashier Note</span><textarea value={paymentNote} onChange={(event) => onPaymentNoteChange(event.target.value)} maxLength={500} placeholder="Reason for reject or retry, optional for approval" /></label>
              </div>
            </div>
          ) : null}
          {isVerified ? (
            <div className="cd-payment-history-strip">
              <span>Verified {order.invoiceVerifiedAt ? fmtDateTime(order.invoiceVerifiedAt) : "-"}</span>
              <span>{order.referenceNumber ? `Ref ${order.referenceNumber}` : "No reference"}</span>
              <span>{order.transactionId ? `Txn ${order.transactionId}` : "No transaction ID"}</span>
            </div>
          ) : null}
          {order.orderNote ? (
        ...
        <div className="cd-drawer-footer">
          ...
          {onReject && <button className="cd-view-btn danger" onClick={onReject} disabled={approving}>Reject</button>}
          {onRetry && <button className="cd-view-btn" onClick={onRetry} disabled={approving}>Request Retry</button>}
        </div>
      </aside>
    </>
  );
...
function CheckoutReceiptPreview({ model }: { model: FinalDiningBillModel }) {
  ...
  return <section className="cd-checkout-receipt" aria-label="Receipt preview">
    <header>{model.restaurant.logoUrl ? <img src={model.restaurant.logoUrl} alt="" /> : <span>{model.restaurant.name.charAt(0)}</span>}<h2>{model.restaurant.name}</h2>{model.restaurant.address ? <p>{model.restaurant.address}</p> : null}{model.restaurant.phone ? <p>{model.restaurant.phone}</p> : null}</header>
    <div className="cd-receipt-meta"><span>Table {model.bill.tableNumber ?? "-"}</span><span>{new Date(model.bill.printedAt).toLocaleDateString()}</span><span>Cashier: {model.bill.cashierName ?? "Cashier"}</span><span>{new Date(model.bill.printedAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</span></div>
    <div className="cd-receipt-lines"><div className="heading"><span>Item</span><span>Qty</span><span>Unit</span><span>Total</span></div>{model.items.map((item, index) => <div key={`${item.name}-${index}`}><span>{item.name}</span><span>{item.quantity}</span><span>{fmtBillMoney(item.unitPrice)}</span><strong>{fmtBillMoney(item.total)}</strong></div>)}</div>
    <div className="cd-receipt-totals"><div><span>Subtotal</span><strong>{fmtBillMoney(model.totals.subtotal)}</strong></div><div><span>VAT ({Math.round(model.totals.vatRate * 100)}%)</span><strong>{fmtBillMoney(model.totals.vatAmount)}</strong></div><div><span>Service Charge</span><strong>{fmtBillMoney(model.totals.serviceChargeAmount)}</strong></div><div><span>Discount</span><strong>- {fmtBillMoney(Math.abs(model.totals.discountAmount))}</strong></div><div className="grand"><span>TOTAL</span><strong>{fmtBillMoney(model.totals.grandTotal)}</strong></div></div>
    <div className="cd-receipt-payment"><small>Payment Method</small><strong>{model.payments.map((payment) => payment.method).join(" + ") || "Verified Payment"}</strong><span>Paid {fmtBillMoney(paid || model.totals.grandTotal)}</span></div>
    <footer><strong>Thank you for dining with us!</strong><span>{printed ? `Printed · Copy ${model.bill.printCount}` : "Review copy · Not yet printed"}</span></footer>
  </section>;
}
...
export function CashierDashboardPage({ restaurantId, restaurant: initialRestaurant, cashierName, initialSection }: CashierDashboardPageProps & { initialSection?: string }) {
  ...
  const [paymentScreenshotPreviewUrl, setPaymentScreenshotPreviewUrl] = useState<string | null>(null);
  const [ownerDuplicateOverride, setOwnerDuplicateOverride] = useState(false);
  const [duplicateReferenceNotice, setDuplicateReferenceNotice] = useState<string | null>(null);
  ...
  const [billFormat, setBillFormat] = useState<FinalBillFormat>(() => {
    const saved = window.localStorage.getItem("serveflow.cashier.receipt-format");
    return saved === "58mm" || saved === "a4" || saved === "browser" ? saved : "80mm";
  });
  const [billWorkingSessionId, setBillWorkingSessionId] = useState<string | null>(null);
  const [lastPrintedBill, setLastPrintedBill] = useState<FinalDiningBillModel | null>(null);
  const [checkoutSession, setCheckoutSession] = useState<DiningSessionSummary | null>(null);
  const [advancedPrinterOptions, setAdvancedPrinterOptions] = useState(false);
  const [billHistory, setBillHistory] = useState<Map<string, BillHistory>>(new Map());
  ...
  useEffect(() => {
    ...
    async function loadScreenshotPreview() {
      ...
      if (!cancelled) setPaymentScreenshotPreviewUrl(error ? null : data.signedUrl);
    }
  ...
  async function checkDuplicateReference(reference: string, transactionId: string) {
    const query = reference.trim() || transactionId.trim();
    if (!query) {
      setDuplicateReferenceNotice(null);
      return;
    }
    const { data, error: lookupError } = await supabase.rpc("find_payment_reference", {
    ...
    const matches = ((data ?? []) as { invoice_id?: string; invoice_number?: number; invoice_display_number?: string | null; verified_at?: string | null }[])
      .filter((row) => row.invoice_id !== drawerOrder?.invoiceId);
    setDuplicateReferenceNotice(matches.length > 0 ? `Reference already exists on ${matches[0].invoice_display_number ?? `invoice ${matches[0].invoice_number ?? "unknown"}`}.` : null);
  }
  ...
  async function handlePaymentScreenshotFileChange(file: File | null) {
    try {
      ...
      if (file.size > PAYMENT_SCREENSHOT_MAX_BYTES) throw new Error("Payment screenshot must be 5 MB or smaller.");
      if (!drawerOrder?.invoiceId) throw new Error("Payment batch is missing for this order.");

      const path = paymentScreenshotPath(restaurantId, drawerOrder.invoiceId, file);
      const { error: uploadError } = await supabase.storage.from(PAYMENT_SCREENSHOT_BUCKET).upload(path, file, {
        cacheControl: "0",
        upsert: false,
        contentType: file.type,
      });
      if (uploadError) throw new Error(uploadError.message);
      const { data, error: signedUrlError } = await supabase.storage.from(PAYMENT_SCREENSHOT_BUCKET).createSignedUrl(path, 60 * 10);
      ...
      setPaymentScreenshotUrl("");
      setPaymentScreenshotPreviewUrl(null);
      setError(uploadError instanceof Error ? uploadError.message : "Payment screenshot upload failed.");
    }
  }
  ...
  async function loadDashboard() {
    const [
      ...
      supabase.from("dining_session_bills").select("dining_session_id,print_count,printed_at").eq("restaurant_id", restaurantId),
    ]);
    ...
    setBillHistory(new Map(((billRows ?? []) as BillHistory[]).map((bill) => [bill.dining_session_id, bill])));
  }
  ...
  async function handleApprove(order: CashierOrder) {
    try {
      ...
      if (!order.invoiceId) throw new Error("Payment batch is missing for this order.");
      setApprovingId(targetActionId);
      setError(null);
      const { data, error: rpcError } = await supabase.rpc("verify_order_payment", {
      ...
      setOrders((prev) => prev.map((existing) => (existing.invoiceId ?? existing.id) === targetActionId ? { ...existing, ...updated, invoiceId: existing.invoiceId, invoiceNumber: existing.invoiceNumber, invoiceStatus: "verified", invoicePaidAt: verifiedAt, invoiceVerifiedAt: verifiedAt, referenceNumber: paymentReference || null, transactionId: paymentTransactionId || null, screenshotUrl: paymentScreenshotUrl || null, items: existing.items } : existing));
      if ((drawerOrder?.invoiceId ?? drawerOrder?.id) === targetActionId) setDrawerOrder((current) => current ? { ...current, ...updated, invoiceId: current.invoiceId, invoiceNumber: current.invoiceNumber, invoiceStatus: "verified", invoicePaidAt: verifiedAt, invoiceVerifiedAt: verifiedAt, referenceNumber: paymentReference || null, transactionId: paymentTransactionId || null, screenshotUrl: paymentScreenshotUrl || null, items: current.items } : null);
      setPaymentReference("");
  ...
  async function handleRejectPayment(order: CashierOrder) {
    try {
      ...
      setError(rejectError instanceof Error ? rejectError.message : "Payment rejection failed.");
    } finally {
      setApprovingId(null);
    }
  }
  ...
  async function handleRequestRetry(order: CashierOrder) {
    try {
      ...
      setApprovingId(targetActionId);
      setError(null);
      const { error: rpcError } = await supabase.rpc("request_order_payment_retry", {
        target_invoice_id: order.invoiceId,
      ...
      setError(retryError instanceof Error ? retryError.message : "Payment retry request failed.");
    } finally {
      setApprovingId(null);
    }
  }
  ...
  async function handlePrintFinalBill(session: DiningSessionSummary) {
    try {
      ...
      setError(null);
      const { data, error: rpcError } = await supabase.rpc("print_final_dining_bill", {
      ...
      setRealtimeNotice(`Receipt printed successfully · Copy ${billModel.bill.printCount}.`);
      await loadDashboard();
    } catch (billError) {
      setError(billError instanceof Error ? billError.message : "Could not print final bill.");
    } finally {
      setBillWorkingSessionId(null);
    }
  }

  async function handleCloseDiningSessionFromBill(session: DiningSessionSummary) {
    if (!window.confirm(`Release Table ${session.tableNumber ?? ""}? Confirm the customer has left and the table is ready for new guests.`)) return;
    try {
      ...
      if (rpcError) throw new Error(rpcError.message);
      setRealtimeNotice(`Table ${session.tableNumber ?? ""} released successfully.`);
      setCheckoutSession(null);
      await loadDashboard();
    } catch (closeError) {
      setError(closeError instanceof Error ? closeError.message : "Could not close dining session.");
    } finally {
      setClosingSessionId(null);
    }
  }

  const verifiedOrders = useMemo(() => orders.filter((order) => order.invoiceStatus === "verified"), [orders]);
  const pendingPayments = useMemo(() => orders.filter(isUnpaidPayment), [orders]);
  ...
  const activeDiningSessions = useMemo(() => buildDiningSessionSummaries(activeOrders), [activeOrders]);
  const completedDiningSessions = useMemo(() => buildDiningSessionSummaries(completedOrders), [completedOrders]);
  const openSessionOrders = useMemo(() => orders.filter(isContinuableOrder), [orders]);
  ...
  const occupiedTableNumbers = useMemo(() => new Set(openSessionOrders.map((order) => order.tableNumber).filter(Boolean)), [openSessionOrders]);
  ...
  const checkoutBillModel = useMemo(() => {
    ...
    if (lastPrintedBill?.bill.diningSessionId === checkoutSession.diningSessionId) return lastPrintedBill;
    return buildFinalBillReviewModel(checkoutSession, restaurant, cashierName || "Cashier", billFormat);
  }, [billFormat, cashierName, checkoutSession, lastPrintedBill, restaurant]);
  const checkoutPrintHistory = checkoutSession ? billHistory.get(checkoutSession.diningSessionId) ?? null : null;
  ...
  return (
    <div className="cd-root">
      <header className="cd-header">
        {realtimeState !== "connected" ? <div role="status" className="cd-realtime-state">Realtime reconnecting…</div> : null}
        ...
        {loading ? (
          ...
          <>
            ...
            <section className="cd-kpi-grid">
              <KpiCard label="Active Orders" value={`${activeDiningSessions.length}`} detail="Active dining sessions" />
              ...
              <div className="cd-card">
                <div className="cd-card-header">
                  <div className="cd-tabs">
                    <button className={`cd-tab${queueTab === "active" ? " active" : ""}`} onClick={() => setQueueTab("active")}>Active Orders <span className="cd-tab-badge">{activeDiningSessions.length}</span></button>
                    ...
                    <button className={`cd-tab${queueTab === "completed" ? " active" : ""}`} onClick={() => setQueueTab("completed")}>Completed Orders <span className="cd-tab-badge">{completedDiningSessions.length}</span></button>
                  </div>
                ...
                <div className="cd-order-list">
                  ...
                  {queueTab === "pending" ? queueOrders.map((order) => {
                      ...
                      <article key={order.invoiceId ?? order.id} className={`cd-order-card ${order.invoiceStatus === "pending" ? "pending_payment" : order.status}`} onClick={() => setDrawerOrder(order)}>
                        ...
                        <div className="cd-order-card-main">
                          <div className="cd-order-card-title">
                            <strong>{fmtOrderLabel(order)} · {fmtInvoiceLabel(order)}</strong>
                            <span className={`cd-badge ${order.invoiceStatus === "verified" ? "paid" : order.invoiceStatus === "rejected" ? "cancelled" : "pending"}`}>{statusLabel(order.invoiceStatus || "pending")}</span>
                            <span className="cd-badge cbe">{statusLabel(order.invoiceKitchenStatus || "waiting_payment")}</span>
                          </div>
                          <div className="cd-order-card-meta">
                            {timeAgo(order.createdAt)} · Created by: {creatorLabel(order)} · {order.items.length} items · {order.paymentMethod || "No method"} · {fmtMoney(order.totalPrice)}
                          </div>
                        ...
                        <div className="cd-order-card-actions" onClick={(event) => event.stopPropagation()}>
                          ...
                          {isVerifiablePayment(order) && <button className="cd-approve-btn" disabled={approvingId === (order.invoiceId ?? order.id)} onClick={() => handleApprove(order)}>{approvingId === (order.invoiceId ?? order.id) ? "..." : "Verify Payment"}</button>}
                        </div>
                      </article>
                    );
                  }) : null}
                  {queueTab !== "pending" && (queueTab === "active" ? activeDiningSessions : completedDiningSessions).length === 0 ? (
                    <div className="cd-empty"><div className="cd-empty-title">No dining sessions in this queue</div><div className="cd-empty-sub">Dining sessions move here by session status.</div></div>
                  ) : null}
                  {queueTab !== "pending" ? (queueTab === "active" ? activeDiningSessions : completedDiningSessions).map((session) => {
                    const firstBatch = session.batches[0];
                    const sessionTotal = session.batches.reduce((sum, batch) => sum + batch.totalPrice, 0);
                    return (
                      <article key={session.diningSessionId} className={`cd-order-card ${queueTab === "completed" ? "completed" : "pending_payment"}`} onClick={() => firstBatch && setDrawerOrder(firstBatch)}>
                        <div className="cd-order-table-tile">Tbl<strong>{session.tableNumber || "-"}</strong></div>
                        <div className="cd-order-card-main">
                          <div className="cd-order-card-title">
                            <strong>{fmtSessionLabel(session)}</strong>
                            <span className={`cd-badge ${session.diningSessionStatus === "open" ? "pending" : "paid"}`}>{statusLabel(session.diningSessionStatus ?? "open")}</span>
                          </div>
                          <div className="cd-order-card-meta">
                            {timeAgo(session.latestAt)} · {session.batches.length} batch{session.batches.length === 1 ? "" : "es"} · {session.itemCount} item{session.itemCount === 1 ? "" : "s"} · {fmtMoney(sessionTotal)}
                          </div>
                          <div className="cd-session-batch-list">
                            {session.batches.map((batch) => {
                              const preview = getOrderItemPreview(batch.items);
                              return (
                                <div className="cd-session-batch" key={batch.invoiceId ?? batch.id}>
                                  <div>
                                    <strong>{fmtOrderLabel(batch)} · {fmtInvoiceLabel(batch)}</strong>
                                    <span>Created by: {creatorLabel(batch)} · {preview.visible.map((item) => `${item.name} x${item.quantity}`).join(", ") || "No items"}{preview.hiddenCount > 0 ? `, +${preview.hiddenCount} more` : ""}</span>
                                  </div>
                                  <div className="cd-session-batch-status">
                                    <span className={`cd-badge ${batch.invoiceStatus === "verified" ? "paid" : batch.invoiceStatus === "rejected" ? "cancelled" : "pending"}`}>{statusLabel(batch.invoiceStatus || "pending")}</span>
                                    <span className="cd-badge cbe">{statusLabel(batch.invoiceKitchenStatus || "waiting_payment")}</span>
                                    <button className="cd-view-btn" type="button" onClick={(event) => { event.stopPropagation(); setDrawerOrder(batch); }}>View</button>
                                  </div>
                                </div>
                              );
                            })}
                ...
                <div className="cd-card">
                  <div className="cd-card-header">
                    <div>
                      ...
                      <div className="cd-card-subtitle">Review the customer bill, print when needed, then release the table manually.</div>
                    </div>
                  </div>
                  <div className="cd-final-bill-panel">
                    {openDiningSessions.length === 0 ? (
                      <div className="cd-empty compact">
                        <div className="cd-empty-title">No tables awaiting checkout</div>
                        <div className="cd-empty-sub">Eligible tables appear after all payments and kitchen items are complete.</div>
                      </div>
                    ) : (
                      <div className="cd-final-bill-session-list">
                        {openDiningSessions.map((session) => {
                          const reasons = [
                            session.pendingCount > 0 ? `${session.pendingCount} payment batch${session.pendingCount === 1 ? "" : "es"} pending` : null,
                            session.incompleteItemCount > 0 ? `${session.incompleteItemCount} kitchen item${session.incompleteItemCount === 1 ? "" : "s"} incomplete` : null,
                            session.verifiedTotal <= 0 ? "No verified payment batches" : null,
                          ].filter(Boolean);
                          const canPrint = reasons.length === 0;
                          return (
                            <article className="cd-final-bill-session" key={session.diningSessionId}>
                              <div className="cd-final-bill-session-top">
                                <div>
                                  <strong>Table {session.tableNumber ?? "-"}</strong>
                                  <span>{customerTypeLabel(session)}</span>
                                </div>
                                <span className={`cd-checkout-status ${canPrint ? "ready" : "blocked"}`}>{canPrint ? "✓ Ready for Checkout" : "Not Yet Eligible"}</span>
                              </div>
                              <div className="cd-checkout-card-summary">
                                <div><small>Opened</small><strong>{fmtTime(session.createdAt)}</strong></div>
                                <div><small>Last Order</small><strong>{fmtTime(session.latestAt)}</strong></div>
                                <div><small>Items</small><strong>{session.itemCount}</strong></div>
                                <div><small>Invoices</small><strong>{session.batches.length}</strong></div>
                              </div>
                              <div className="cd-checkout-paid"><small>Paid Amount</small><strong>{fmtMoney(session.verifiedTotal)}</strong><span>{[...new Set(session.batches.filter((batch) => batch.invoiceStatus === "verified").map((batch) => batch.paymentMethod || "Other"))].join(" + ")}</span></div>
                              {reasons.length > 0 ? <div className="cd-final-bill-warning">{reasons.join(". ")}</div> : null}
                              <button
                                type="button"
                                className="cd-review-bill-btn"
                                disabled={!canPrint}
                                onClick={() => { setCheckoutSession(session); setAdvancedPrinterOptions(false); }}
                              >
                                Review Bill
                              </button>
                            </article>
                  ...
                  <div className="cd-pos-menu">
                    ...
                    {selectedTableActiveOrder && (
                      <div className="cd-pos-active-note">
                        Active order {fmtOrderLabel(selectedTableActiveOrder)} found for Table {selectedTable}. Submitting will ask whether to add to it or create a new order.
                      </div>
                    )}
    ...
    <OrderDrawer
          ...
          onApprove={isVerifiablePayment(drawerOrder) ? () => handleApprove(drawerOrder) : undefined}
          onReject={isVerifiablePayment(drawerOrder) ? () => handleRejectPayment(drawerOrder) : undefined}
          onRetry={drawerOrder.invoiceStatus === "pending" || drawerOrder.invoiceStatus === "rejected" ? () => handleRequestRetry(drawerOrder) : undefined}
          ...
          onPaymentReferenceChange={(value) => void handlePaymentReferenceChange(value)}
          onPaymentTransactionIdChange={(value) => void handlePaymentTransactionIdChange(value)}
      ...
      {checkoutSession && checkoutBillModel && (
        <div className="cd-checkout-overlay">
          <section className="cd-checkout-dialog" role="dialog" aria-modal="true" aria-label={`Checkout Table ${checkoutSession.tableNumber ?? ""}`}>
            <header className="cd-checkout-header"><button type="button" onClick={() => setCheckoutSession(null)}>← Back</button><div><strong>Table {checkoutSession.tableNumber ?? "-"}</strong><span>✓ Ready for Checkout</span></div><button type="button" onClick={() => setCheckoutSession(null)} aria-label="Close checkout">×</button></header>
            <div className="cd-checkout-layout">
              <aside className="cd-checkout-summary"><h2>Table Summary</h2><dl><div><dt>Table</dt><dd>{checkoutSession.tableNumber ?? "-"}</dd></div><div><dt>Customer Type</dt><dd>{customerTypeLabel(checkoutSession)}</dd></div><div><dt>Opened</dt><dd>{fmtTime(checkoutSession.createdAt)}</dd></div><div><dt>Last Order</dt><dd>{fmtTime(checkoutSession.latestAt)}</dd></div><div><dt>Total Items</dt><dd>{checkoutSession.itemCount}</dd></div><div><dt>Invoices</dt><dd>{checkoutSession.batches.length}</dd></div></dl><div className="cd-checkout-payment"><small>Payment Status</small><strong>{fmtMoney(checkoutSession.verifiedTotal)}</strong><span>Paid via {checkoutBillModel.payments.map((payment) => payment.method).join(" + ") || "verified payment"}</span></div><p>All payments and kitchen items have been cleared. Printing is optional; release the table only after the customer leaves.</p></aside>
              <div className="cd-checkout-preview-wrap"><div className="cd-preview-label">Receipt Preview</div><CheckoutReceiptPreview model={checkoutBillModel} /></div>
              <aside className="cd-print-controls"><h2>{checkoutPrintHistory || checkoutBillModel.bill.printCount > 0 ? "Receipt Printed" : "Ready to Print"}</h2>{checkoutPrintHistory || checkoutBillModel.bill.printCount > 0 ? <div className="cd-print-history"><span>Printed</span><strong>{checkoutPrintHistory?.print_count ?? checkoutBillModel.bill.printCount} {(checkoutPrintHistory?.print_count ?? checkoutBillModel.bill.printCount) === 1 ? "copy" : "copies"}</strong><small>Last printed {fmtDateTime(checkoutPrintHistory?.printed_at ?? checkoutBillModel.bill.printedAt)}</small></div> : <p>Review the bill carefully before printing.</p>}<button type="button" className="cd-advanced-print" onClick={() => setAdvancedPrinterOptions((open) => !open)}>Advanced Printer Options <span>{advancedPrinterOptions ? "⌃" : "⌄"}</span></button>{advancedPrinterOptions ? <div className="cd-bill-format-toggle">{(["80mm", "58mm", "a4", "browser"] as FinalBillFormat[]).map((format) => <button key={format} type="button" className={billFormat === format ? "active" : ""} onClick={() => setBillFormat(format)}>{format === "a4" ? "A4" : format === "browser" ? "Browser" : `${format} Thermal`}</button>)}</div> : <div className="cd-printer-current">Printer: {billFormat === "a4" ? "A4" : billFormat === "browser" ? "Browser" : `${billFormat} Thermal`}</div>}</aside>
            </div>
            <footer className="cd-checkout-actions"><button type="button" className="cd-release-table-btn" onClick={() => void handleCloseDiningSessionFromBill(checkoutSession)} disabled={closingSessionId === checkoutSession.diningSessionId}>{closingSessionId === checkoutSession.diningSessionId ? "Releasing…" : "Release Table"}</button><button type="button" className="cd-print-receipt-btn" onClick={() => void handlePrintFinalBill(checkoutSession)} disabled={billWorkingSessionId === checkoutSession.diningSessionId}>{billWorkingSessionId === checkoutSession.diningSessionId ? "Preparing Receipt…" : checkoutPrintHistory || checkoutBillModel.bill.printCount > 0 ? "Reprint Receipt" : "Print Receipt"}</button></footer>
          </section>
        </div>
      )}
          ...
          <div className="cd-modal" role="dialog" aria-modal="true" aria-label="Active order found">
            <div className="cd-modal-header">
              <div>
                ...
                <p>{fmtOrderLabel(continuationChoice.activeOrder)} is {statusLabel(continuationChoice.activeOrder.status)} with a current total of {fmtMoney(continuationChoice.activeOrder.totalPrice)}.</p>
              </div>
