...
export function InventoryOperationalDashboard({ requests, requestsLoading, requestsError, canProcessRequests, requestStorageLocations, working, onIssue, onUnable }: Props) {
  ...
  return <div className="ia-kr-page">
    <header className="ia-kr-heading"><div><h2>Kitchen Requests</h2><p>Review and issue materials requested by kitchen.</p></div>{accepted.length > 0 && <strong>{accepted.length} awaiting inventory</strong>}</header>
        ...
        : visibleRequests.length === 0 ? <div className="ia-kr-state"><strong>{tab === "accepted" ? "No requests are awaiting Inventory." : tab === "issued" ? "No requests are awaiting Kitchen." : "No Kitchen request history yet."}</strong></div>
          : <div className="ia-kr-list">{visibleRequests.map((request) => {
            ...
            return <article key={request.id} className={`ia-kr-card ${active ? availability : request.status}`}>
              ...
              {active && <>
                <div className="ia-kr-requested"><span>Requested quantity</span><strong>{formatInventoryQuantity(request.quantity, request.unit)}</strong></div>
                ...
                <div className="ia-kr-meta"><span>Requested by {request.requesterName ?? "Name not recorded"}</span><time dateTime={request.requestedAt}>{dateTimeLabel(request.requestedAt)}</time></div>
    ...
    {action && <div className="ia-kr-backdrop" role="presentation" onClick={closeAction}><section className="ia-kr-dialog" role="dialog" aria-modal="true" aria-labelledby="ia-kr-dialog-title" onClick={(event) => event.stopPropagation()}><header><div><span>KITCHEN REQUEST</span><h2 id="ia-kr-dialog-title">{action.kind === "issue" ? `Issue ${action.request.itemName}` : "Cannot Fulfill"}</h2><p>{action.request.stationName ?? "Kitchen / station not recorded"}</p></div><button type="button" aria-label="Close request dialog" disabled={working} onClick={closeAction}>×</button></header>
