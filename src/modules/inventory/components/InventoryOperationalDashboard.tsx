  ...
  return (
    <div className="ia-stack ia-i1-dashboard">
        ...
        {!requestsError && !requestsLoading && visibleRequests.length > 0 && (
          <div className="ia-i1-request-list">
            {visibleRequests.map((request) => {
              ...
              const unavailable = request.status === "accepted" && (!request.inventoryItemId || available === null || !hasUsableUnit);
              const canIssue = canProcessRequests && request.status === "accepted" && Boolean(request.inventoryItemId) && !insufficient && !unavailable;
              ...
              return <article key={request.id} className={`ia-i1-request-card ${cardState}`}>
                <header><div className="ia-i1-request-primary"><small>{materialRequestTypeLabel(request.requestType)}</small><strong>{request.itemName || "Unnamed material"}</strong><b>{quantityLabel(request.quantity, request.unit)}</b></div>{showHistoryStatus && <span className={`ia-i1-status ${request.status}`}>{request.status === "delivered" ? "Received by Kitchen" : request.status === "unable_to_fulfill" ? "Unable to Fulfill" : "Rejected"}</span>}{!showHistoryStatus && cardState === "priority" && <span className="ia-i1-priority">{request.urgency === "critical" ? "Critical" : "High"} priority</span>}</header>
                <strong className="ia-i1-station">{request.stationName ?? "Station not recorded"}</strong>
                <div className="ia-i1-request-meta"><span>Requested by {request.requesterName ?? "Name not recorded"}</span><time dateTime={request.requestedAt}>{dateTimeLabel(request.requestedAt)}</time></div>
      ...
      {action && <div className="ia-i1-dialog-backdrop" role="presentation" onClick={() => !working && setAction(null)}><section className="ia-i1-dialog" role="dialog" aria-modal="true" aria-labelledby="i1-dialog-title" onClick={(event) => event.stopPropagation()}>
        <header><div><span>KITCHEN REQUEST</span><h2 id="i1-dialog-title">{action.kind === "issue" ? "Issue Stock" : "Cannot Fulfill Request"}</h2><p>{action.request.itemName || "Inventory item"} → {action.request.stationName ?? "Station not recorded"}</p></div><button type="button" disabled={working} onClick={() => setAction(null)} aria-label="Close request dialog">×</button></header>
