...
const staffRoleLabel = (value: unknown) => str(value) === "kitchen" ? "Chef" : str(value);
...
export function managerReportsExportRows(report: ManagerReportsV1Bundle): ExportRow[] {
  ...
  report.operations.staff.facts.forEach((value) => { const row = record(value); rows.push(["Staff Operations", str(row.display_name), staffRoleLabel(row.role), num(row.orders_created) + num(row.kitchen_items_completed) + num(row.inventory_movements) + num(row.cashier_shifts_opened), "Recorded workload events; no performance score"]); });
