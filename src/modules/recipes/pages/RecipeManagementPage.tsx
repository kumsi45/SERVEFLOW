...
const menuImageUrl = (item: { id: string; image_url?: string | null }) => resolveMenuItemImage({ itemId: item.id, master: item.image_url ? { source: "MASTER", status: "APPROVED", url: item.image_url, version: 1 } : null, placeholderUrl: "" }).url;
...
export function RecipeManagementPage({ restaurantId, role }: { restaurantId: string; role: RecipeRole }) {
  ...
  return <main className="recipe-page recipe-v1">
    ...
    {step > 0 && editable && <div className="recipe-modal" role="dialog" aria-modal="true"><section className="recipe-builder v1-builder"><header><div><small>{editing ? "Edit Recipe" : `Create Recipe • Step ${step} of 6`}</small><h2>{step === 1 ? "Select Menu Item" : step === 2 ? "Recipe Name" : step === 3 ? "Ingredients" : step === 4 ? "Preparation Time" : step === 5 ? "Status" : "Save Recipe"}</h2></div><button className="secondary" onClick={resetWizard}>Cancel</button></header><div className="wizard-progress">{[1,2,3,4,5,6].map((number) => <span key={number} className={step >= number ? "active" : ""} />)}</div>
      {step === 1 && <><input autoFocus aria-label="Search menu items" placeholder="Search menu items" value={menuSearch} onChange={(event) => setMenuSearch(event.target.value)} /><div className="menu-picker-list v1-menu-list">{menuChoices.map((item) => <button key={item.id} onClick={() => chooseMenu(item)}>{menuImageUrl(item) ? <img src={menuImageUrl(item) ?? ""} alt="" loading="lazy" /> : <span>{item.name.slice(0, 1)}</span>}<div><strong>{item.name}</strong><small>{item.category_name ?? "Uncategorized"} • {money(item.price)} ETB</small></div><b>Choose →</b></button>)}{menuChoices.length === 0 && <div className="recipe-empty">No available menu items match.</div>}</div></>}
      {step === 2 && <div className="wizard-screen"><div className="selected-menu compact">{selectedMenu && menuImageUrl(selectedMenu) ? <img src={menuImageUrl(selectedMenu) ?? ""} alt="" /> : <span>{selectedMenu?.name.slice(0,1) ?? draft.name.slice(0,1)}</span>}<div><small>{selectedMenu?.category_name}</small><strong>{selectedMenu?.name ?? draft.name}</strong></div></div><label>Recipe Name<input autoFocus required value={draft.name} onChange={(event) => setDraft({ ...draft, name: event.target.value })} /></label><button className="wizard-next" disabled={!draft.name.trim()} onClick={() => setStep(3)}>Continue</button></div>}
