...
export function PurchaseOrderDraftsPage({ restaurantId, suppliers, items, units, storageLocations }: Props) {
  ...
  return <div className="po-page">
    ...
    {form && <div className="po-backdrop" role="presentation" onClick={() => !working && setForm(null)}><section className="po-editor" role="dialog" aria-modal="true" aria-labelledby="po-editor-title" onClick={(event) => event.stopPropagation()}>
      <header className="po-editor-heading"><div><h2 id="po-editor-title">{form.id ? "Edit Purchase Order" : "Create Purchase Order"}</h2></div><button type="button" aria-label="Close purchase order" onClick={() => setForm(null)}>&times;</button></header>
      <div className="po-header-fields"><label>Supplier <span>(optional)</span><select value={form.supplierId} onChange={(event) => setForm({ ...form, supplierId: event.target.value })}><option value="">Select supplier</option>{activeSuppliers.map((supplier) => <option key={supplier.id} value={supplier.id}>{supplier.name}</option>)}</select></label><label>Expected delivery<input required type="date" value={form.expectedDeliveryDate} onChange={(event) => setForm({ ...form, expectedDeliveryDate: event.target.value })} /></label></div>
