...
export const PremiumGridCard = memo(function PremiumGridCard({
  item,
  priority = false,
  onAddToCart,
  onOpenInfo,
  ...
  const ratingValue = (
    ...
    item as MenuItem & { average_rating?: number | null }
  ).average_rating;
  const rating =
  ...
  return (
    <article
      className={`premium-grid-card${item.available ? "" : " unavailable"}`}
    >
      <button
        className="premium-grid-card-media"
        type="button"
      ...
      <div className="premium-grid-card-body">
        <div className="premium-grid-card-heading">
          <h3>{item.name}</h3>
          <div className="premium-grid-card-meta">
        ...
        <div className="premium-grid-card-actions">
          <button
            className="premium-grid-info"
            type="button"
            onClick={() => onOpenInfo(item)}
