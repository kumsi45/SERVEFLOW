...
export const ThemeCustomizationStudio = memo(function ThemeCustomizationStudio({
  restaurantId,
  role,
  onPublished,
}: ThemeCustomizationStudioProps) {
  const [restaurant, setRestaurant] = useState<RestaurantStudioRow | null>(
    null,
  );
  const [publishedTheme, setPublishedTheme] = useState<MenuTheme>("modern");
  const [selectedTheme, setSelectedTheme] = useState<MenuTheme>("modern");
  ...
  const definitions = useMemo(() => Object.values(themeRegistry), []);
  const dirty = useMemo(
    () =>
      selectedTheme !== publishedTheme ||
      JSON.stringify(customization) !== JSON.stringify(publishedCustomization),
    [customization, publishedCustomization, publishedTheme, selectedTheme],
  );
  ...
  async function publish() {
    ...
    try {
      ...
      setError(null);
      setNotice(null);
      const storedCustomization = createStoredThemeCustomization(customization);
  ...
  function confirmReset() {
      if (resetAction === "theme") {
        ...
        setNotice(
          role === "owner"
            ? "Theme reset prepared. Publish to make it live."
            : "Theme reset prepared in this draft.",
        );
  ...
  return (
    ...
    >
      <header className="tcs-studio-header">
        ...
        <button
          className="tcs-preview-action"
          type="button"
          onClick={focusPreview}
          disabled={working}
        >
          <Eye size={17} aria-hidden /> Preview
        </button>
      </header>
      ...
      <div className="tcs-editor-layout">
        ...
        />
        <div
          className={`tcs-preview-column${previewOpen ? " mobile-open" : ""}`}
          ref={previewRef}
          tabIndex={-1}
          role={previewOpen ? "dialog" : undefined}
          aria-modal={previewOpen ? true : undefined}
          aria-label={previewOpen ? "Menu preview" : undefined}
        >
          <div className="tcs-preview-heading">
            <h3>Preview</h3>
            <button
              className="tcs-preview-close"
              type="button"
      ...
      {dirty && (
        <footer className="tcs-studio-actions">
          <details className="tcs-more-actions">
            <summary aria-label="More appearance actions">
          ...
          <div className="tcs-publish-actions">
            ...
            <button
              className={role === "manager" ? "primary" : undefined}
              type="button"
              onClick={saveDraft}
              disabled={working}
            >
        ...
        <div className="tcs-dialog-layer" role="presentation">
          ...
          >
            ...
            <div>
              ...
              <button className="danger" type="button" onClick={confirmReset}>
                Confirm Reset
              </button>
            </div>
          </section>
