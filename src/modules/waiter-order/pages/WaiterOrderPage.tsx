...
function navigateWaiter(path:string,replace=false){replace?window.history.replaceState({},"",path):window.history.pushState({},"",path);window.dispatchEvent(new PopStateEvent("popstate"));}
...
export function WaiterOrderPage({ restaurantSlug, tableNumber }: Props) {
  ...
  async function submit() { if (!cart.items.length) return;const clientRequestId=crypto.randomUUID();const payload={clientRequestId,restaurantSlug,tableNumber,customerName,customerPhone:session?.customerPhone??"",orderNote,items:cart.items};try { setSubmitting(true);setError(null);if(!navigator.onLine){queueWaiterOrder(payload);cart.clearCart();navigateWaiter("/waiter/dashboard",true);return}await submitWaiterOrder(payload);cart.clearCart();navigateWaiter(`/waiter/dashboard?table=${encodeURIComponent(tableNumber)}`,true); } catch(e){if(!navigator.onLine||e instanceof TypeError){queueWaiterOrder(payload);cart.clearCart();navigateWaiter("/waiter/dashboard",true);return}setError(e instanceof Error?e.message:"Order could not be submitted.");} finally{setSubmitting(false);} }
  ...
  return <main className="w93-page">
    ...
    <div className="w93-layout"><section className="w93-catalog">
      ...
      {!menu.searchTerm && menu.activeCategoryId === "all" && quickSections.map((section) => <section className="w93-quick" key={section.title}><h2>{section.title}</h2><div>{section.items.slice(0,8).map((item) => <button key={item.id} onClick={() => add(item)}><span>{item.name}</span><strong>+ {formatETBPrice(Number(item.price))}</strong></button>)}</div></section>)}
      <section className="w93-menu-grid">{menu.groups.flatMap((group) => group.items).map((item) => <article key={item.id} className={!item.available ? "unavailable" : ""}><button className="w93-favorite" onClick={() => toggleFavorite(item.id)} aria-label="Favorite">{favorites.includes(item.id)?"★":"☆"}</button><button className="w93-item-main" disabled={!item.available} onClick={() => add(item)}>{item.effective_image_url || item.image_url ? <img src={item.effective_image_url ?? item.image_url ?? ""} alt="" /> : <span className="w93-placeholder">🍽</span>}<span className="w93-item-copy"><strong>{item.name}</strong><small>{item.description || "Tap to add"}</small><b>{formatETBPrice(Number(item.price))}</b></span><i>＋</i></button><button className="w93-customize" onClick={() => customize(item)} disabled={!item.available}>Modifiers & Notes</button></article>)}</section>
    </section><aside className="w93-cart"><div className="w93-cart-title"><div><small>Current Order</small><h2>Table {tableNumber}</h2></div><strong>{cart.itemCount}</strong></div><div className="w93-cart-lines">{cart.items.length?cart.items.map((line) => <article key={line.menuItemId}><div><strong>{line.name}</strong><small>{line.notes || "No instructions"}</small></div><b>{formatETBPrice(line.price*line.quantity)}</b><div className="w93-qty"><button onClick={() => line.quantity===1?cart.removeItem(line.menuItemId):cart.updateQuantity(line.menuItemId,line.quantity-1)}>−</button><strong>{line.quantity}</strong><button onClick={() => cart.updateQuantity(line.menuItemId,line.quantity+1)}>+</button><button onClick={() => customize(itemById.get(line.menuItemId)!)}>✎</button></div></article>):<div className="w93-empty">Tap an item to add it instantly.</div>}</div><label className="w93-field"><span>Customer</span><input value={customerName} onChange={(e)=>setCustomerName(e.target.value)} placeholder="Optional name" /></label><label className="w93-field"><span>Special instructions for this order</span><textarea value={orderNote} onChange={(e)=>setOrderNote(e.target.value)} placeholder="Kitchen or service notes" /></label>{error&&<div className="w93-error">{error}</div>}<div className="w93-total"><span>Total</span><strong>{formatETBPrice(cart.displaySubtotal)}</strong></div><button className="w93-submit" disabled={!cart.items.length||submitting} onClick={() => void submit()}>{submitting?"Sending…":session?.order_id?"Send Added Items":"Send Order"}</button></aside></div>
    {modifierItem && <div className="w93-sheet-bg" onClick={() => setModifierItem(null)}><section className="w93-sheet" onClick={(e)=>e.stopPropagation()}><header><div><small>Customize item</small><h2>{modifierItem.name}</h2></div><button onClick={()=>setModifierItem(null)}>×</button></header><div className="w93-sheet-qty"><span>Quantity</span><div><button onClick={()=>setModifierQuantity((q)=>Math.max(1,q-1))}>−</button><strong>{modifierQuantity}</strong><button onClick={()=>setModifierQuantity((q)=>Math.min(99,q+1))}>+</button></div></div><div className="w93-presets">{NOTE_PRESETS.map((note)=><button key={note} className={modifierNotes.includes(note)?"active":""} onClick={()=>setModifierNotes((current)=>current.includes(note)?current.split(" · ").filter((value)=>value!==note).join(" · "):[current,note].filter(Boolean).join(" · "))}>{note}</button>)}</div><label><span>Kitchen notes / special instructions</span><textarea autoFocus value={modifierNotes} onChange={(e)=>setModifierNotes(e.target.value)} placeholder="Preparation details, allergy notes, substitutions…" /></label><button className="w93-save" onClick={saveCustomization}>Save Item · {formatETBPrice(Number(modifierItem.price)*modifierQuantity)}</button></section></div>}
  </main>;
}
