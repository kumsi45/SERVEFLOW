...
export async function searchInventoryItems(restaurantId:string,query:string){
  ...
  if(normalizedQuery)request=request.ilike("name",`%${normalizedQuery.replace(/[%_]/g,"")}%`);
  const {data,error}=await request;
  if(error)throw new Error(error.message);
  return (data??[]).map(row=>({id:row.id,name:row.name,unit:row.unit,currentQuantity:Number(row.current_quantity),reorderLevel:Number(row.reorder_level),active:row.active})) as InventoryItem[];
}

export async function loadInventoryItems(restaurantId:string){
...
export async function createInventoryRequest(restaurantId:string,input:{requestType:MaterialRequestType;itemName:string;quantity:number;unit:string;urgency:InventoryUrgency;stationId?:string|null;comment?:string;inventoryItemId?:string|null}){
  const {error}=await supabase.rpc("create_kitchen_inventory_request",{
    target_restaurant_id:restaurantId,
    target_item_name:input.itemName,
    target_quantity:input.quantity,
    target_unit:input.unit,
    target_urgency:input.urgency,
    target_station_id:input.stationId??null,
...
export function materialRequestErrorMessage(error:unknown){
  const message=error instanceof Error?error.message:String(error??"");
  if(/inventory item.*required|item is required/i.test(message))return "Select an item.";
  if(/item name.*required/i.test(message))return "Enter a material name.";
  if(/quantity/i.test(message))return "Enter a quantity greater than 0.";
  if(/unit/i.test(message))return "Select a unit.";
  if(/station/i.test(message))return "Your Kitchen station is unavailable.";
  if(/access denied|permission|not authorized/i.test(message))return "You do not have access to create this request.";
  return "Unable to submit the request. Try again.";
}

export function materialRequestTypeLabel(type:MaterialRequestType){
  if(type==="ingredient")return "Ingredient / Food Material";
  if(type==="supply")return "Kitchen Supply";
  if(type==="tool")return "Tool / Equipment";
  if(type==="cleaning")return "Cleaning / Consumable";
  return "Other";
}
