...
export function AiMenuFinalPreview({ restaurant: sourceRestaurant, state, draftVersion, lastUpdated, onReturn, onPublish, publishing }: Props) {
  ...
  return <section className="menu-final-preview" aria-label="Final digital menu preview">
    ...
    <div className="menu-preview-controls" aria-label="Preview controls">
      ...
      <div className="menu-supported-languages" aria-label="Supported languages"><span>Supported Languages</span><strong>✓ English</strong><strong>✓ Amharic</strong><strong>✓ Afaan Oromoo</strong></div>
    </div>
    <div className="menu-preview-certification">
      <section className="menu-ready-checklist" aria-labelledby="menu-ready-title"><header><div><span>Pre-publish certification</span><h3 id="menu-ready-title">Ready to publish</h3></div><strong className={certification.canPublish ? "ready" : "attention"}>{certification.canPublish ? "Required setup complete" : "Required setup incomplete"}</strong></header><div>{certification.checks.map((check) => <article className={check.ready ? "ready" : check.blocking ? "attention" : "optional"} key={check.id}><span aria-hidden="true">{check.ready ? "✓" : check.blocking ? "•" : "○"}</span><div><strong>{check.label}</strong><small>{check.detail}</small></div></article>)}</div></section>
