...
export function certifyMenuPreview(restaurant: MenuPreviewRestaurant, state: MenuReviewState) {
  ...
  const checks: PreviewCheck[] = [
    { id: "restaurant", label: "Restaurant Name", detail: restaurant.name.trim() ? restaurant.name : "Restaurant name is missing", ready: Boolean(restaurant.name.trim()), blocking: true },
    { id: "logo", label: "Restaurant Logo", detail: restaurant.logo_url ? "Logo ready" : "Logo not added", ready: Boolean(restaurant.logo_url), blocking: false },
    { id: "cover", label: "Restaurant Cover", detail: restaurant.cover_url ? "Cover ready" : "Cover not added", ready: Boolean(restaurant.cover_url), blocking: false },
    ...
    { id: "tracking", label: "Tracking Type", detail: invalidTrackingTypes ? `${invalidTrackingTypes} items need a tracking type` : "All items configured", ready: invalidTrackingTypes === 0, blocking: true },
