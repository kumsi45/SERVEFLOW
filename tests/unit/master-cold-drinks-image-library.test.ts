...
const manifest = JSON.parse(readFileSync(resolve(root, "src/modules/setup-wizard/data/masterColdDrinksImageLibrary.v1.json"), "utf8"));
...
const categories = ["Fresh Juice", "Milkshakes", "Cold Drinks", "Soft Drinks"];
const approved = specs.filter((entry: { active: boolean; category: string }) => entry.active && categories.includes(entry.category));

describe("Phase 9.13.10 Cold Drinks master images", () => {
  it("contains exactly every active approved drink", () => {
    expect(manifest.phase).toBe("9.13.10");
    ...
    expect(manifest.images.filter((entry: { category: string }) => entry.category === "Fresh Juice")).toHaveLength(5);
    expect(manifest.images.filter((entry: { category: string }) => entry.category === "Milkshakes")).toHaveLength(5);
    expect(manifest.images.filter((entry: { category: string }) => entry.category === "Cold Drinks")).toHaveLength(3);
    expect(manifest.images.filter((entry: { category: string }) => entry.category === "Soft Drinks")).toHaveLength(1);
  });
  ...
  it("provides 84 immutable valid unique WebP variants", () => {
    const checksums = new Set<string>();
    const paths = new Set<string>();
    for (const entry of manifest.images) {
      expect(entry.responsive_variants.map((variant: { width: number }) => variant.width)).toEqual([2048, 1280, 1024, 768, 512, 320]);
      expect(entry.base_storage_path).toMatch(/^(cafe\/(fresh-juice|smoothies-milkshakes)|bar-lounge\/soft-drinks|bakery\/fresh-juice|hotel\/soft-drinks)\/[a-z0-9-]+$/);
