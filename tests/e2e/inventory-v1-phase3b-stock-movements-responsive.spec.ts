...
const markup = `<main class="ia-sm-page"><header class="ia-sm-heading"><div><span>OPERATIONAL HISTORY</span><h2>Stock Movements</h2><p>What changed in stock, where, and when</p></div><button>Refresh</button></header><section class="ia-sm-tools"><label><span>Search movements</span><input placeholder="Search materials, storage, staff..."></label><button>Filters<strong>2</strong></button></section><div class="ia-sm-mobile-list">${cards}</div><div class="ia-sm-desktop-table"><table><thead><tr><th>Date / Time</th><th>Movement</th><th>Material</th><th>Storage</th><th>Quantity</th><th>Source / Reason</th><th>Staff</th><th>Actions</th></tr></thead><tbody>${tableRows}</tbody></table></div><div class="ia-sm-load-more"><span>Showing 25 of 48</span><button>Load More</button></div></main>`;
