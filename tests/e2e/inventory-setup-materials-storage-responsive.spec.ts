...
const fixture = `<main><div class="ia-setup-page ia-materials-page"><header class="ia-setup-heading"><div><h2>Materials</h2><p>Materials tracked by this business.</p></div><button>Add Material</button></header><div class="ia-setup-tools"><label><span>Search materials</span><input placeholder="Search materials"></label><details><summary>Filters</summary><div><label>Category<select><option>All categories</option></select></label><label>Status<select><option>Active</option></select></label></div></details></div><section class="ia-material-list" aria-label="Materials"><div class="ia-material-row header"><span>Material</span><span>Category</span><span>Unit</span><span>Status</span><span>Action</span></div>${material("Coffee", "kg")}${material("Very Long Housekeeping Cleaning Chemical Material Name That Must Wrap", "litres")}${material("Tissue", "box", "archived")}</section></div><hr><div class="ia-setup-page ia-storage-page"><header class="ia-setup-heading"><div><h2>Storage</h2><p>Places where inventory materials are kept.</p></div><button>Add Storage</button></header><section class="ia-storage-grid" aria-label="Storage locations">${storage("Main Store", "Primary dry-goods storage.", 7)}${storage("Very Long Housekeeping and Guest Supplies Storage Location", "", 0)}${storage("Freezer", "", 2)}${storage("Old Bar Store", "", 0, "archived")}</section></div></main>`;
