...
function markup(operation: Operation) {
  ...
  const primary = transfer
    ? `<label>Material<select><option>Extra Fine Imported Brown Sugar With A Very Long Material Name</option></select></label><div class="ia-so-auto-storage"><span>From</span><strong>Main Beverage and Dry Goods Storage</strong><small>60 kg available</small></div><label>To storage<select><option>Bar and Beverage Store</option></select></label><label>Quantity<input inputmode="decimal" value="12.375"></label>`
    : `<label>Material<select><option>Extra Fine Imported Brown Sugar With A Very Long Material Name</option></select></label><div class="ia-so-auto-storage"><span>Storage</span><strong>Main Beverage and Dry Goods Storage</strong>${operation === "Stock Out" ? "<small>60 kg available</small>" : ""}</div><label>Quantity<input inputmode="decimal" value="12.375"></label>`;
  const supplier = operation === "Stock In" ? `<label>Supplier<select><option>Select supplier (optional)</option><option>General Supplier</option></select></label>` : "";
  return `<main class="ia-so-page"><header><div><h2>${operation}</h2></div></header><form class="ia-so-form"><div class="ia-so-primary-fields">${primary}</div><div class="ia-so-stock-context"><span>${operation === "Stock In" ? "Current stock" : "Available"}</span><strong>26.125 kg</strong></div><details class="ia-so-details"><summary>Additional details</summary><div>${supplier}<label>Reason<textarea class="ia-so-reason" rows="2" placeholder="Why is this stock being changed? (optional)"></textarea></label><label>Movement time<input type="datetime-local" value="2026-08-23T17:41"></label></div></details><footer><button>Review ${operation}</button></footer></form></main>`;
}
...
const viewports = [[360, 800], [375, 812], [390, 844], [412, 915], [430, 932], [768, 1024], [1024, 768], [1440, 900]];
...
for (const [width, height] of viewports) {
  for (const operation of operations) {
    test(`${operation} cleanup fits ${width}x${height}`, async ({ page }) => {
      ...
      await expect(page.locator(".ia-so-auto-storage")).toBeVisible();
