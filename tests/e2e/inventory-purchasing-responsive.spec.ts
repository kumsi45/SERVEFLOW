...
const markup = `<main><div class="po-page"><header class="po-heading"><div><h2>Purchase Orders</h2><p>Track orders and receive deliveries.</p></div><button>Create Purchase Order</button></header><div class="po-tabs"><button aria-selected="true">Open<span>1</span></button><button>Partially Received<span>1</span></button><button>Completed<span>1</span></button><button>All<span>3</span></button></div><details class="po-filters"><summary>Search and filters</summary></details><section class="po-order-grid">${purchaseCard("draft", 1)}${purchaseCard("partially_received", 2)}${purchaseCard("completed", 3)}</section></div><hr><div class="ia-suppliers-page"><header class="ia-suppliers-heading"><div><h2>Suppliers</h2><p>Manage the businesses you buy materials from.</p></div><button>Add Supplier</button></header><label class="ia-suppliers-search"><span>Search suppliers</span><input placeholder="Search name, phone, or contact"></label><section class="ia-suppliers-grid">${supplierCard(1, true)}${supplierCard(2, false)}${supplierCard(3, false)}</section></div></main>`;
