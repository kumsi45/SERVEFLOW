...
const stockMarkup = `<main class="ia-cs-page"><section class="ia-cs-tools"><label class="ia-cs-search"><span>Search stock</span><input placeholder="Material, storage, or category"></label><button class="ia-cs-filter-button">Filters<strong>2</strong></button><button class="ia-cs-action-button">+ Stock Action</button></section><div class="ia-cs-mobile-list">${mobileRows}</div><div class="ia-cs-desktop-table"><table><thead><tr><th>Material</th><th>Current</th><th>Minimum</th><th>Maximum</th><th>Storage</th><th>Status</th><th>Actions</th></tr></thead><tbody>${desktopRows}</tbody></table></div></main>`;

const operationMarkup = `<main class="ia-so-page"><header><div><h2>Stock In</h2></div></header><form class="ia-so-form"><div class="ia-so-primary-fields"><label>Material<select><option>Extra Fine Imported Brown Sugar With A Very Long Material Name</option></select></label><label>Storage<select><option>Main Beverage and Dry Goods Storage</option></select></label><label>Quantity<input inputmode="decimal" value="12.375"></label></div><div class="ia-so-stock-context"><span>Current stock</span><strong>26.125 kg</strong></div><details class="ia-so-details"><summary>Additional details</summary><div><label>Supplier<select><option>General Supplier</option></select></label><label>Reason<textarea class="ia-so-reason" rows="2" placeholder="Why is this stock being changed? (optional)"></textarea></label><label>Movement time<input type="datetime-local"></label></div></details><footer><button>Review Stock In</button></footer></form></main>`;
...
for (const viewport of viewports) {
  test(`Inventory Phase 3 Current Stock fits ${viewport.width}x${viewport.height}`, async ({ page }) => {
    ...
    await expect(page.getByPlaceholder("Material, storage, or category")).toBeVisible();
...
test("Inventory Phase 3 mobile sheets stay in the viewport", async ({ page }) => {
  await load(page, 360, 800, `${stockMarkup}<div class="ia-cs-sheet-backdrop"><section class="ia-cs-sheet"><header><h3>Update Stock</h3><button aria-label="Close">×</button></header><div class="ia-cs-action-list"><button><strong>Receive</strong></button><button><strong>Issue</strong></button><button><strong>Transfer</strong></button><button><strong>Details</strong></button></div></section></div>`);
