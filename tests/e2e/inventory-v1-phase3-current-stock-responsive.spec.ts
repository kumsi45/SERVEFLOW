...
const stockMarkup = `<main class="ia-cs-page"><header class="ia-cs-heading"><div><h2>Current Stock</h2></div><button>+ Stock Action</button></header><section class="ia-cs-tools"><label class="ia-cs-search"><span>Search stock</span><input placeholder="Material, storage, or category"></label><button class="ia-cs-filter-button">Filters<strong>2</strong></button></section><div class="ia-cs-mobile-list">${mobileRows}</div><div class="ia-cs-desktop-table"><table><thead><tr><th>Material</th><th>Current</th><th>Minimum</th><th>Maximum</th><th>Storage</th><th>Status</th><th>Actions</th></tr></thead><tbody>${desktopRows}</tbody></table></div></main>`;
